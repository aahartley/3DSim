...
void mouseEventHandler(int button, int state, int x, int y) {
  // get the shift key status to pass to the camera
  bool shiftkey = (glutGetModifiers() == GLUT_ACTIVE_SHIFT);
  // let the camera handle mouse button events (similar to maya)
  camera->HandleMouseEvent(button, state, x, y, shiftkey);
  glutPostRedisplay();
}

void motionEventHandler(int x, int y) {
  // let the camera handle some mouse motions events to move the camera
