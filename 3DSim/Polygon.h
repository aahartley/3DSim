...
struct Triangle : public Polygon {
	...
	float zmax = 0;
	Triangle() {};
	Triangle(std::vector<Vector3f> vertices, Vector3f origin) : vertices(vertices), origin(origin) {
		surfaceNormal = (vertices[1] - vertices[0]).cross((vertices[2] - vertices[1]));
		surfaceNormalU = (vertices[1] - vertices[0]).cross((vertices[2] - vertices[1])).normalize();
		xmin = vertices[0].x;
		xmax = vertices[0].x;
		ymin = vertices[0].y;
		ymax = vertices[0].y;
